...
export default function NotificationsModal({ 
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="reminders-panel">
      ...
      <div className="lg:col-span-6 bg-white p-6 rounded-2xl border border-sky-100 shadow-sm space-y-4 text-xs h-[34rem] flex flex-col justify-between">
        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          <div className="flex items-center justify-between pb-3 border-b">
            ...
            <span className="px-2.5 py-0.5 bg-rose-100 text-rose-800 text-[10px] font-bold rounded-xl">
            ...
            {alerts.filter(a => !a.read).map(alert => (
              ...
              >
                ...
                <div className="flex-1 pr-6">
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider block opacity-70 mb-0.5">
                    {alert.category} • {alert.severity}
                  </span>
                  <p className="font-bold leading-relaxed text-xs">{alert.message}</p>
                  <span className="text-[10px] font-black font-mono block mt-1.5 opacity-50">
                ...
                <button
                  ...
                  className="absolute right-3 top-3.5 p-1 text-slate-400 hover:text-slate-800 hover:bg-slate-100/50 rounded-xl cursor-pointer transition-colors"
        ...
        <div className="p-3 bg-indigo-50 border border-indigo-100 text-indigo-950 font-bold rounded-xl flex gap-3 mt-4 items-start">
      ...
      <div className="lg:col-span-6 bg-white p-6 rounded-2xl border border-sky-100 shadow-sm space-y-4 text-xs h-[34rem] overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-3 border-b">
          ...
          <span className="px-2.5 py-0.5 bg-sky-100 text-sky-850 text-[10px] font-bold rounded-xl font-mono">
            ...
            >
              <div className="space-y-1.5 flex-1 pr-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 bg-slate-100 text-slate-500 font-mono font-bold text-[10px] rounded uppercase">
                    {(notif.type || '').replace('_',' ')}
                  </span>
                </div>

                <p className="text-slate-600 leading-relaxed font-bold">{notif.message}</p>
              ...
              <div>
                {notif.status === 'queued' ? (
                  <button
                    ...
                    className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs active:scale-95 text-[10px]"
                  >
                    <Send className="h-3.5 w-3.5" />
                    <span>Send SMS/Mail</span>
                  </button>
                ) : (
                  <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 text-[10px] font-bold uppercase rounded flex items-center gap-1">
