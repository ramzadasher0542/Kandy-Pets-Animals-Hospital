      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col border border-slate-200 rounded-2xl bg-white shadow-sm relative animate-fade-in">
        ...
        {selectedClient.client_status === 'flagged_bad_debt' && (
          <div className="bg-rose-50 border-b border-rose-200 p-4 flex items-center gap-3 shrink-0 sticky top-0 z-10">
            ...
            <div>
              <h4 className="text-sm font-bold text-rose-700">RESTRICTED ACCOUNT: BAD DEBT</h4>
            ...
            <div className="space-y-4">
              <div>
                <h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                  ...
                  }} className="p-1.5 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 rounded-xl transition-colors cursor-pointer" title="Edit Client Profile">
              ...
              {selectedClient.physical_address && (
                <div className="flex items-start gap-1.5 text-xs font-bold text-slate-500 max-w-sm">
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      <div className="flex justify-between items-start mb-2">
                        <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-slate-100 px-2.5 py-1 rounded">{pet.petType}</div>
                      ...
                      <div className="text-xs font-bold text-slate-500 truncate mb-1">{pet.breed || 'Unknown Breed'}</div>
                      <div className="text-[10px] font-black text-slate-400 truncate mb-4">Age: {pet.age || 'Unknown'}</div>
                      
                      <div className="mt-auto">
                        <button className="w-full py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-[10px] font-bold uppercase tracking-widest rounded-xl transition-colors cursor-pointer border border-indigo-100 flex items-center justify-center gap-1.5">
                  ...
                  clientAppointments.map(apt => (
                    <div key={apt.id} className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
                      <div>
                        ...
                        <div className="text-[10px] font-black text-slate-500 mt-0.5">{apt.petName}</div>
                      </div>
                      <div className="text-right">
                        <div className={`text-[10px] px-1.5 py-0.5 rounded font-black uppercase mt-0.5 ${
        ...
        <div className="bg-white border-b border-slate-200 shrink-0">
          <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100 bg-slate-50/50">
            ...
            <div className="flex items-center gap-2">
               {onGenerateConsent && (
                 <button onClick={() => onGenerateConsent(selectedClient.full_name, activePet.name)} className="px-4 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-[10px] font-black uppercase tracking-widest rounded-xl transition-colors flex items-center gap-1.5 shadow-xs cursor-pointer">
          ...
          <div className="flex px-6 gap-2">
            <button onClick={() => setPassportTab('timeline')} className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-t-xl transition-colors flex items-center gap-2 border-b-0 ${passportTab === 'timeline' ? 'bg-slate-50 text-indigo-600 border border-slate-200' : 'bg-white text-slate-400 hover:bg-slate-50'}`}>
              <Activity className="w-3.5 h-3.5"/> Clinical Timeline
            </button>
            <button onClick={() => setPassportTab('exams')} className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-t-xl transition-colors flex items-center gap-2 border-b-0 ${passportTab === 'exams' ? 'bg-slate-50 text-indigo-600 border border-slate-200' : 'bg-white text-slate-400 hover:bg-slate-50'}`}>
              <HeartPulse className="w-3.5 h-3.5"/> Systemic Exams
            </button>
            <button onClick={() => setPassportTab('labs')} className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-t-xl transition-colors flex items-center gap-2 border-b-0 ${passportTab === 'labs' ? 'bg-slate-50 text-indigo-600 border border-slate-200' : 'bg-white text-slate-400 hover:bg-slate-50'}`}>
              <TestTube className="w-3.5 h-3.5"/> Laboratory
            </button>
            <button onClick={() => setPassportTab('vaccines')} className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-t-xl transition-colors flex items-center gap-2 border-b-0 ${passportTab === 'vaccines' ? 'bg-slate-50 text-indigo-600 border border-slate-200' : 'bg-white text-slate-400 hover:bg-slate-50'}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {abnormalSystems.map(([systemKey, data]: [string, any]) => (
                          <div key={systemKey} className="bg-rose-50/50 p-3 rounded-xl border border-rose-100">
                            ...
                            {data.notes && <div className="text-[10px] font-black text-rose-700 mt-2 italic">"{data.notes}"</div>}
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-400 uppercase tracking-widest font-black text-[10px]">
                    ...
                    <th className="py-4 px-6">Results / Notes</th>
                  </tr>
                </thead>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest mb-1.5">Primary Phone *</label>
                        <PhoneInput
                          name="primary_phone"
                          required
                      ...
                      <div>
                        <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest mb-1.5">Email Address</label>
                      ...
                      <div>
                        <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest mb-1.5">Physical Address</label>
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest mb-1.5">Age</label>
            ...
            <form onSubmit={handleUpdateExistingClient} className="flex flex-col min-h-0">
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1 col-span-1">
                    <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest">Primary Phone *</label>
                    <PhoneInput required value={formData.primary_phone} onChange={val => setFormData({...formData, primary_phone: val})} />
                  </div>
                  <div className="space-y-1 col-span-1">
                    <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest">Alternate Phone</label>
                    <PhoneInput value={formData.alternate_phone} onChange={val => setFormData({...formData, alternate_phone: val})} />
                  </div>
                  <div className="space-y-1 col-span-2">
                    <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest">Email Address</label>
                  ...
                  <div className="space-y-1 col-span-2">
                    <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest">Physical Address</label>
                  ...
                  <div className="space-y-1 col-span-2">
                    <label className="font-bold text-slate-500 block text-[10px] uppercase tracking-widest">Administrative Notes</label>
                  ...
                  {deleteTarget.hadHistory ? (
                    <div data-testid="delete-history-warning" className="bg-rose-50 border border-rose-200 rounded-xl p-4 space-y-3">
                      ...
                      <label className="flex items-start gap-2 cursor-pointer select-none">
                        ...
                        />
                        <span className="text-[10px] font-bold text-rose-800">I understand this {label} has history and want to delete anyway.</span>
                      </label>
                    </div>
                  ) : (
